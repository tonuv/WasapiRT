...
HRESULT __stdcall Wasapi::AudioInterfaceActivator::ActivateCompleted(IActivateAudioInterfaceAsyncOperation* pActivateOperation)
{
	...
	if (FAILED(hrActivate)) {
		...
		activationCompleted.set(activatedInterface.as<IAudioClient>());
	}
	return S_OK;;
}

concurrency::task<winrt::com_ptr<::IAudioClient>> Wasapi::AudioInterfaceActivator::ActivateAudioInterfaceAsync(LPCWCHAR szDeviceId)
{
